...
async function run() {
  const sql = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260921010000_add_queries_to_audit_projects.sql'), 'utf-8');
  ...
  const { data: testData, error: testError } = await supabase
    .from('audit_projects')
    .select('id, mom_data, mom_columns, testing_data, testing_columns, queries_data, queries_columns')
    .limit(1);

  if (testError) {
    console.log("Column select check:", testError.message);
  } else {
    console.log("✓ Successfully verified queries_data, queries_columns in audit_projects!");
  }
}
